---
# Customer Management (CRM) module patterns - profiles, loyalty points, wallet, referrals, segmentation, and preferences
inclusion: fileMatch
fileMatchPattern: '**/customer/**/*.ts, **/crm/**/*.ts, **/loyalty/**/*.ts'
---
...
POST /api/v1/customers:
  ...
  response: { customer: Customer, referralApplied: boolean }
...
GET /api/v1/customers:
  ...
  query:
    ...
    page?: number
    limit?: number
  response: { customers: Customer[], meta: PaginationMeta }

GET /api/v1/customers/:id:
  description: Get customer details
  auth: required
  response: { customer: Customer, stats: CustomerStats, recentVisits: CustomerVisit[] }
...
GET /api/v1/customers/search:
  description: Search by phone/name/email
  auth: required
  query:
    query: string
    limit?: number
    includeInactive?: boolean
  response: { customers: CustomerSummary[], total: number }

GET /api/v1/customers/lookup/:phone:
  description: Quick lookup by phone
  auth: required
...
GET /api/v1/customers/:id/visits:
  ...
  query:
    page?: number
    limit?: number
  response: { visits: CustomerVisit[], meta: PaginationMeta }

GET /api/v1/customers/:id/timeline:
  description: Get engagement timeline
  auth: required
...
POST /api/v1/customers/:id/notes:
  ...
  auth: required
  body: { noteType: NoteType, content: string, isPinned?: boolean }
  response: { note: CustomerNote }

GET /api/v1/customers/:id/notes:
  description: Get notes
  auth: required
  response: { notes: CustomerNote[] }

PATCH /api/v1/customers/:id/preferences:
  description: Update preferences
  auth: required
  body: { preferences: { type: string, value: string }[] }
  response: { preferences: CustomerPreference[] }
...
GET /api/v1/customers/:id/loyalty:
  ...
  auth: required
  response: LoyaltySummaryResponse
...
GET /api/v1/customers/:id/loyalty/history:
  ...
  query:
    page?: number
    limit?: number
  response: { transactions: LoyaltyPointsTransaction[], meta: PaginationMeta }

POST /api/v1/customers/:id/loyalty/earn:
  description: Manual points credit
  auth: required (branch_manager+)
  body: { points: number, reason: string }
  response: { transaction: LoyaltyPointsTransaction }

POST /api/v1/customers/:id/loyalty/redeem:
  description: Redeem points
  auth: required
  body: { points: number, redemptionType: 'discount' | 'service', invoiceId?: string }
  response: { transaction: LoyaltyPointsTransaction, discountValue: number }
```
...
GET /api/v1/customers/:id/wallet/history:
  description: Get wallet history
  auth: required
  query:
    page?: number
    limit?: number
  response: { transactions: WalletTransaction[], meta: PaginationMeta }
...
POST /api/v1/customers/:id/wallet/debit:
  description: Debit from wallet
  auth: required
  body: { amount: number, invoiceId?: string, description?: string }
  response: { transaction: WalletTransaction }
```
...
```yaml
GET /api/v1/customers/:id/referral-code:
  description: Get/generate referral code
  auth: required
  response: { referralCode: string }

GET /api/v1/customers/:id/referrals:
  description: Get referral history
  auth: required
  response: { referrals: Referral[], stats: { total: number, completed: number } }

POST /api/v1/referrals/validate:
  description: Validate referral code
  auth: public
  body: { code: string }
  response: { valid: boolean, referrer?: { name: string } }

POST /api/v1/referrals/apply:
  description: Apply referral to new customer
  auth: required
  body: { customerId: string, referralCode: string }
...
POST /api/v1/customer-segments:
  ...
  response: { segment: CustomerSegment }

PATCH /api/v1/customer-segments/:id:
  description: Update segment
  auth: required (branch_manager+)
  body: Partial<CreateSegmentRequest>
  response: { segment: CustomerSegment }

DELETE /api/v1/customer-segments/:id:
  description: Delete segment
  auth: required (branch_manager+)
  response: { success: true }

POST /api/v1/customer-segments/:id/compute:
  description: Recompute segment members
  auth: required
  response: { segment: CustomerSegment, customerCount: number }

GET /api/v1/customer-segments/:id/members:
  description: Get segment members
  auth: required
  query:
    page?: number
    limit?: number
  response: { customers: CustomerSummary[], meta: PaginationMeta }
```
...
class LoyaltyEngine {
  ...
  async redeemPoints(
    ...
    if (customer.loyaltyPointsBalance < points) {
      throw new BadRequestError('INSUFFICIENT_POINTS', 'Not enough loyalty points');
    }
  ...
  async processExpiredPoints(): Promise<void> {
    ...
    const expiringTransactions = await this.db.loyaltyPointsTransactions.findMany({
      where: {
    ...
    for (const { customerId, _sum } of expiringTransactions) {
      ...
      // Update balance
      await this.db.customers.update(customerId, {
        loyaltyPointsBalance: Math.max(0, customer.loyaltyPointsBalance - expiringPoints)
      });

      // Notify customer
      await this.notificationService.sendPointsExpired(customer, expiringPoints);
    }
  }
...
class ReferralService {
   ...
   */
  async applyReferral(newCustomerId: string, referralCode: string): Promise<Referral> {
    ...
    if (!referrer) {
      throw new NotFoundError('INVALID_REFERRAL_CODE', 'Referral code not found');
    }
    ...
    if (existingReferral) {
      throw new ConflictError('ALREADY_REFERRED', 'Customer already has a referral');
    }
   ...
   */
  private async awardReferrerReward(tx: Transaction, referral: Referral): Promise<void> {
...
class SegmentationEngine {
  ...
  private buildCondition(condition: SegmentCondition): any {
    ...
    if (field === 'last_visit_days_ago') {
      ...
      return this.buildOperatorCondition('lastVisitDate', this.invertOperator(operator), cutoffDate);
    }

    if (field === 'age') {
      const birthYear = new Date().getFullYear() - value;
      return this.buildOperatorCondition('dateOfBirth', this.invertOperator(operator), `${birthYear}-01-01`);
    }
  ...
  async getSystemSegments(): Promise<CustomerSegment[]> {
    return [
      {
        name: 'New Customers',
        rules: { logic: 'AND', conditions: [{ field: 'total_visits', operator: 'eq', value: 1 }] }
      },
      {
        name: 'Regular Customers',
        rules: { logic: 'AND', conditions: [{ field: 'total_visits', operator: 'gte', value: 5 }] }
      },
      {
        name: 'VIP Customers',
        rules: { logic: 'AND', conditions: [{ field: 'lifetime_value', operator: 'gte', value: 50000 }] }
      },
      {
        name: 'Inactive (30+ days)',
        rules: { logic: 'AND', conditions: [{ field: 'last_visit_days_ago', operator: 'gte', value: 30 }] }
      },
      {
        name: 'At Risk (60+ days)',
        rules: { logic: 'AND', conditions: [{ field: 'last_visit_days_ago', operator: 'gte', value: 60 }] }
      },
      {
        name: 'Churned (90+ days)',
        rules: { logic: 'AND', conditions: [{ field: 'last_visit_days_ago', operator: 'gte', value: 90 }] }
      }
    ];
  }
}
...
export const createNoteSchema = z.object({
  noteType: z.enum(['general', 'preference', 'allergy', 'complaint', 'follow_up']),
...
describe('Customer Flow Integration', () => {
  it('should complete full customer lifecycle: create → visit → earn points → redeem');
  it('should complete referral flow: apply code → first purchase → reward both');
