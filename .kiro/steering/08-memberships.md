---
# Memberships & Packages module patterns - membership plans, prepaid packages, benefits, redemption, and validity
inclusion: fileMatch
fileMatchPattern: '**/membership/**/*.ts, **/package/**/*.ts, **/subscription/**/*.ts'
---

# Memberships & Packages Module
...
class MembershipPackageRefundService {
  ...
  async cancelMembership(
    ...
    if (config.refundPolicy === RefundPolicy.REFUNDABLE) {
      ...
      cancellationFee = membership.pricePaid * (config.cancellationFeePercentage / 100);
...
export const createMembershipPlanSchema = z.object({
  ...
  benefits: z.array(z.object({
    benefitType: z.enum([
      'flat_discount', 'service_discount', 'product_discount',
      'complimentary_service', 'priority_booking', 'visit_limit',
      'cooldown_period', 'benefit_cap', 'fallback_discount'
    ]),
    ...
    complimentaryPeriod: z.enum(['per_visit', 'per_month', 'per_year', 'total']).optional(),
  ...
  data => !(data.branchScope === 'specific_branches' && (!data.branchIds || data.branchIds.length === 0)),
