---
# Tenant Management module patterns - business registration, branches, users, roles, subscriptions, and theming
inclusion: fileMatch
fileMatchPattern: '**/tenant/**/*.ts, **/auth/**/*.ts, **/branch/**/*.ts, **/user/**/*.ts'
---
...
const PERMISSIONS: Record<string, UserRole[]> = {
  // Tenant
  'tenant:read': ['super_owner', 'regional_manager', 'branch_manager', 'accountant'],
  'tenant:write': ['super_owner'],

  // Branches
  'branches:read': ['super_owner', 'regional_manager', 'branch_manager', 'receptionist', 'stylist', 'accountant'],
...
// Branch access check
async function checkBranchAccess(userId: string, branchId: string): Promise<boolean> {
...
async function checkSubscriptionLimits(
  ...
  if (action === 'branch' && tenant._count.branches >= tenant.maxBranches) {
    throw new PaymentRequiredError('Branch limit reached. Please upgrade your plan.');
  }

  if (action === 'user' && tenant._count.users >= tenant.maxUsers) {
    throw new PaymentRequiredError('User limit reached. Please upgrade your plan.');
  }
}
...
async function checkTrialStatus(tenant: Tenant): Promise<void> {
  if (tenant.subscriptionStatus === 'trial' && tenant.trialEndsAt < new Date()) {
    throw new PaymentRequiredError('Trial period has ended. Please subscribe to continue.');
  }
}
```
...
async function resolveTheme(
  ...
  // Precedence: User > Branch > Tenant > Default
  ...
  if (userPref) {
    return {
      config: {
        preset: userPref.preset,
        appearance: userPref.appearance,
        customColors: userPref.customPrimary ? {
          primary: userPref.customPrimary,
          primaryLight: userPref.customPrimaryLight,
          primaryDark: userPref.customPrimaryDark,
        } : undefined,
      },
      source: 'user',
    };
  ...
  // 2. Check branch theme
  if (branchId) {
    const branchTheme = await db.branchThemeSettings.findUnique({
      where: { branchId },
    });
    if (branchTheme) {
      return {
        config: {
          preset: branchTheme.preset,
          appearance: branchTheme.appearance,
          customColors: branchTheme.customPrimary ? {
            primary: branchTheme.customPrimary,
            primaryLight: branchTheme.customPrimaryLight,
            primaryDark: branchTheme.customPrimaryDark,
          } : undefined,
        },
        source: 'branch',
      };
  ...
  // 3. Check tenant theme
  if (tenantId) {
    const tenantTheme = await db.tenantThemeSettings.findUnique({
      where: { tenantId },
    });
    if (tenantTheme) {
      return {
        config: {
          preset: tenantTheme.preset,
          appearance: tenantTheme.appearance,
          customColors: tenantTheme.customPrimary ? {
            primary: tenantTheme.customPrimary,
            primaryLight: tenantTheme.customPrimaryLight,
            primaryDark: tenantTheme.customPrimaryDark,
          } : undefined,
        },
        source: 'tenant',
      };
...
export const branchSettingsSchema = z.object({
  ...
  cancellationWindowHours: z.number().min(0).max(48).optional(),
  gracePeriodMinutes: z.number().min(0).max(60).optional(),
  pricesIncludeGst: z.boolean().optional(),
  roundingRule: z.enum(['nearest_1', 'nearest_5', 'nearest_10']).optional(),
...
export const createUserSchema = z.object({
  ...
  employmentType: z.enum(['full_time', 'part_time', 'contractual']).optional(),
});
...
export const updateUserSchema = z.object({
  ...
  role: z.enum(['regional_manager', 'branch_manager', 'receptionist', 'stylist', 'accountant']).optional(),
...
export const updateTenantSchema = z.object({
  ...
  invoiceHeader: z.string().max(500).optional(),
...
| All Modules             | Tenant isolation via RLS, audit logging      |
...
```typescript
// errors/tenant-errors.ts

import { NotFoundError, ConflictError, ForbiddenError, PaymentRequiredError } from './base-errors';
...
export class BranchLimitExceededError extends PaymentRequiredError {
...
export class UserLimitExceededError extends PaymentRequiredError {
  constructor(max: number) {
...
export class InvalidSubscriptionPlanError extends BadRequestError {
  constructor() {
    super('Invalid subscription plan');
    this.code = 'TEN_006';
  }
}
...
export class BranchCodeExistsError extends ConflictError {
  constructor() {
    super('Branch code already exists');
    this.code = 'BRN_002';
  }
}
...
export class UserEmailExistsError extends ConflictError {
  constructor() {
    super('Email already exists in tenant');
    this.code = 'USR_002';
  }
}
...
|---------|--------------------------------|-------------|
| TEN_001 | Tenant not found               | 404         |
| TEN_002 | Email already registered       | 409         |
| TEN_003 | Branch limit reached           | 402         |
| TEN_004 | User limit reached             | 402         |
| TEN_005 | Subscription expired           | 403         |
| TEN_006 | Invalid subscription plan      | 400         |
| BRN_001 | Branch not found               | 404         |
| BRN_002 | Branch code already exists     | 409         |
| USR_001 | User not found                 | 404         |
| USR_002 | Email already exists in tenant | 409         |
| USR_003 | Invalid credentials            | 401         |
| USR_004 | Account deactivated            | 403         |
