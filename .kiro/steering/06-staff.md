---
# Staff Management module patterns - profiles, attendance, shifts, leaves, commissions, salary, payroll, and performance tracking
inclusion: fileMatch
fileMatchPattern: '**/staff/**/*.ts, **/attendance/**/*.ts, **/payroll/**/*.ts, **/leave/**/*.ts, **/commission/**/*.ts'
---
...
CREATE POLICY tenant_isolation ON staff_branch_assignments
  FOR ALL USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
CREATE POLICY tenant_isolation ON staff_shift_assignments
  FOR ALL USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
CREATE POLICY tenant_isolation ON staff_deductions
  FOR ALL USING (tenant_id = current_setting('app.current_tenant_id')::uuid);
...
class AttendanceService {
  ...
  async checkIn(
    ...
    if (!shift) {
      throw new BadRequestError('NO_SHIFT_ASSIGNED', 'No shift assigned for today');
    }
...
class LeaveService {
  ...
  async applyLeave(
    ...
    // Notify manager
    await this.notificationService.notifyLeaveRequest(leave, appointments.length);
...
class CommissionService {
  ...
  async getCommissionSummary(
    ...
    const byService = commissions.reduce((acc, c) => {
...
class PayrollService {
  ...
  private async calculateStaffPayroll(
    ...
    for (const component of salaryStructure.filter(c => c.componentType === 'earning')) {
      const amount = this.calculateComponent(component, staff.baseSalary, attendance);
    ...
    for (const component of salaryStructure.filter(c => c.componentType === 'deduction')) {
      const amount = this.calculateComponent(component, staff.baseSalary, attendance);
    ...
    for (const ded of activeDeductions) {
      deductions[ded.deductionType] = (deductions[ded.deductionType] || 0) + ded.monthlyDeduction;
    ...
    // Calculate overtime
    const overtimeRate = (staff.baseSalary / (attendance.workingDays * 8)) * 1.5;
...
export const createStaffSchema = z.object({
  ...
  branchAssignments: z.array(z.object({
    ...
    isPrimary: z.boolean(),
    workingHoursOverride: z.record(z.object({
