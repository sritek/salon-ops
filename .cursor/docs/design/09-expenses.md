# Module 9: Expenses & Finance - Design Document
...
```
┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│ Financial_Year  │──────<│    Expense      │>──────│ Expense_Category│
│                 │  1:N  │                 │  N:1  │                 │
└─────────────────┘       └────────┬────────┘       └─────────────────┘
                                   │
                    ┌──────────────┼──────────────┐
                    ...
                    ▼              ▼              ▼
          ┌─────────────┐  ┌─────────────┐  ┌─────────────┐
          │  Expense_   │  │  Expense_   │  │   Vendor    │
          │ Allocations │  │ Attachments │  │             │
          └─────────────┘  └─────────────┘  └─────────────┘

┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│ Recurring_      │       │   Petty_Cash    │       │ Period_Closure  │
│ Expense         │       │                 │       │                 │
└─────────────────┘       └─────────────────┘       └─────────────────┘
...
CREATE TABLE vendors (
  ...
  -- Payment terms
  payment_terms_days INTEGER DEFAULT 0, -- 0 = immediate
...
expense: Expense;
requiresApproval: boolean;
requiresBackdateApproval: boolean;
budgetWarning?: {
categoryName: string;
budgetedAmount: number;
currentSpend: number;
...
interface ApproveExpenseRequest {
  approvalNotes?: string;
  approveBackdate?: boolean; // If expense is backdated
}
...
```typescript
// POST /api/v1/petty-cash/replenish
interface ReplenishPettyCashRequest {
  branchId: string;
  amount: number;
  description?: string;
}

interface ReplenishPettyCashResponse {
  success: boolean;
  data: {
...
// POST /api/v1/petty-cash/reconcile
interface ReconcilePettyCashRequest {
  branchId: string;
  actualBalance: number;
  notes?: string;
}

interface ReconcilePettyCashResponse {
  success: boolean;
  data: {
...
```typescript
// POST /api/v1/period-closures/close
...
interface CreateOpeningBalanceRequest {
  branchId: string;
  financialYearId: string;
  balanceType: OpeningBalanceType;
  amount: number;
  vendorId?: string; // For accounts_payable
  description?: string;
}

interface CreateOpeningBalanceResponse {
  success: boolean;
  data: {
...
interface CashFlowReportRequest {
  branchId?: string;
  startDate: string;
  endDate: string;
...
- Calculate GST breakdown
  ...
  return {
  gstAmount: Math.round(gstAmount _ 100) / 100,
  cgstAmount: Math.round(cgstAmount _ 100) / 100,
  sgstAmount: Math.round(sgstAmount _ 100) / 100,
  igstAmount: Math.round(igstAmount _ 100) / 100,
...
- Send reminders for upcoming recurring expenses
  ...
  const recurringExpenses = await this.recurringExpenseRepo.findUpcoming();

  for (const recurring of recurringExpenses) {
  const daysUntilDue = this.getDaysDifference(new Date(), recurring.nextDueDate);

      if (daysUntilDue <= recurring.reminderDaysBefore) {
...
- Get petty cash register report
  \*/
  async getRegister(
  branchId: string,
  startDate: string,
  endDate: string
  ): Promise<PettyCashRegister> {
  const transactions = await this.pettyCashRepo.findByDateRange(
  branchId, startDate, endDate
  );

  const openingBalance = await this.getBalanceAtDate(branchId, startDate);
  ...
  totalOut: Math.abs(transactions.filter(t => t.amount < 0).reduce((sum, t) => sum + t.amount, 0)),
  };
...
### 6. Financial Year Service

```typescript
class FinancialYearService {
  /**
   * Auto-create new financial year
   */
  async autoCreateFinancialYear(): Promise<FinancialYear | null> {
    const config = await this.financeConfigService.getConfig();
    const today = new Date();

    // Check if we're at the start of a new FY
    if (today.getMonth() + 1 !== config.fyStartMonth || today.getDate() !== 1) {
      return null;
    }

    // Check if FY already exists
    const existingFY = await this.getForDate(today);
    if (existingFY) {
      return null;
    }
    ...
    const endYear = config.fyStartMonth === 1 ? startYear : startYear + 1;

    const yearCode = `FY${startYear}-${endYear.toString().slice(-2)}`;
    const yearName = `Financial Year ${startYear}-${endYear.toString().slice(-2)}`;

    const startDate = new Date(startYear, config.fyStartMonth - 1, 1);
    const endDate = new Date(endYear, config.fyStartMonth - 1, 0);

    const fy = await this.financialYearRepo.create({
      yearCode,
      yearName,
    ...
    await this.carryForwardBalances(fy.id);
   ...
   */
  private async carryForwardBalances(newFYId: string): Promise<void> {
    const previousFY = await this.getPreviousFinancialYear(newFYId);
    ...
    for (const branch of branches) {
      // Carry forward petty cash balance
      const pettyCashBalance = await this.pettyCashService.getBalance(
        branch.id,
      );

      await this.openingBalanceRepo.create({
        branchId: branch.id,
        financialYearId: newFYId,
        balanceType: OpeningBalanceType.PETTY_CASH,
        amount: pettyCashBalance.currentBalance,
        isLocked: true,
        lockedAt: new Date(),
      });

      // Carry forward outstanding payables
      const pendingPayments = await this.expenseRepo.findPendingPayments(
        branch.id,
      );
      for (const expense of pendingPayments) {
        await this.openingBalanceRepo.create({
          branchId: branch.id,
          financialYearId: newFYId,
          balanceType: OpeningBalanceType.ACCOUNTS_PAYABLE,
          amount: expense.totalAmount,
          vendorId: expense.vendorId,
          description: `Carried forward from ${previousFY.yearCode}`,
          isLocked: true,
          lockedAt: new Date(),
        });
      }
...
### 7. Salary Integration Service

```typescript
class SalaryIntegrationService {
  /**
   * Create expense from approved payroll
   */
  async createFromPayroll(payrollId: string): Promise<Expense> {
    const payroll = await this.payrollRepo.findById(payrollId);

    if (payroll.status !== "approved") {
      throw new ExpenseError(
        "INVALID_PAYROLL_STATUS",
        "Payroll must be approved to create expense",
      );
    }

    // Get salary category
    const salaryCategory =
      await this.expenseCategoryRepo.findByCode("SALARIES_WAGES");

    // Create expense
    const expense = await this.expenseRepo.create({
      branchId: payroll.branchId,
      expenseDate: payroll.paymentDate,
      categoryId: salaryCategory.id,
      baseAmount: payroll.totalAmount,
      gstAmount: 0,
      totalAmount: payroll.totalAmount,
      description: `Salary - ${payroll.staffName} - ${payroll.periodMonth}/${payroll.periodYear}`,
      paymentMethod: ExpensePaymentMethod.BANK_TRANSFER,
  ...
  /**
   * Reverse salary expense if payroll is reversed
   */
  async reverseFromPayroll(payrollId: string): Promise<void> {
    const expense = await this.expenseRepo.findBySource(
      ExpenseSourceType.PAYROLL,
      payrollId,
    );
...
### 1. Billing Module Integration

```typescript
// Revenue data for P&L
interface BillingIntegration {
  // Get revenue by type for period
  getRevenueByType(branchId: string, startDate: string, endDate: string): Promise<{
    serviceRevenue: number;
    productRevenue: number;
  }>;

  // Get commissions total for period
  getCommissionsTotal(branchId: string, startDate: string, endDate: string): Promise<number>;

  // Get refunds total for cash flow
  getRefundsTotal(branchId: string, startDate: string, endDate: string): Promise<number>;
}
````

### 2. Staff Module Integration

```typescript
// Salary expense integration
interface StaffIntegration {
  // Listen for payroll approval
  onPayrollApproved(payrollId: string): Promise<void>;

  // Listen for payroll reversal
  onPayrollReversed(payrollId: string): Promise<void>;
}
```
...
interface InventoryIntegration {
  // Get consumables usage for period
  getConsumptionTotal(
    branchId: string,
    startDate: string,
    endDate: string,
...
```typescript
// Sales revenue for P&L
interface MembershipPackageIntegration {
  // Get membership sales for period
  getMembershipSalesTotal(
    branchId: string,
    startDate: string,
    endDate: string,
  ): Promise<number>;
...
```sql
-- Expense queries
CREATE INDEX idx_expenses_date_status ON expenses(branch_id, expense_date, status);
CREATE INDEX idx_expenses_category_date ON expenses(category_id, expense_date);
CREATE INDEX idx_expenses_vendor_date ON expenses(vendor_id, expense_date);
CREATE INDEX idx_expenses_fy_branch ON expenses(financial_year_id, branch_id);

-- Recurring expense queries
CREATE INDEX idx_recurring_due ON recurring_expenses(next_due_date, status)
  WHERE status = 'active';

-- Petty cash queries
CREATE INDEX idx_petty_cash_branch_date ON petty_cash(branch_id, transaction_date);

-- Budget queries
CREATE INDEX idx_budgets_period ON budgets(branch_id, budget_year, budget_month);
```

### Caching Strategy

```typescript
// Cache financial year (rarely changes)
const FINANCIAL_YEAR_CACHE_TTL = 3600; // 1 hour

// Cache expense categories (rarely changes)
const EXPENSE_CATEGORIES_CACHE_TTL = 1800; // 30 minutes

// Cache finance config (rarely changes)
const FINANCE_CONFIG_CACHE_TTL = 3600; // 1 hour

// Cache budget summary (invalidate on expense create)
const BUDGET_SUMMARY_CACHE_TTL = 300; // 5 minutes
```

### Query Optimization

```typescript
// Use materialized view for P&L calculations
CREATE MATERIALIZED VIEW mv_daily_revenue AS
SELECT
  branch_id,
  invoice_date,
  SUM(CASE WHEN item_type = 'service' THEN net_amount ELSE 0 END) as service_revenue,
  SUM(CASE WHEN item_type = 'product' THEN net_amount ELSE 0 END) as product_revenue,
...
// Hidden from roles
const HIDDEN_FROM_ROLES = {
  Receptionist: ["report.pl", "report.cash_flow", "budget.manage"],
  Stylist: ["expense.*", "petty_cash.*", "report.*", "budget.*"],
};
```
...
// Audit log retention
const FINANCE_AUDIT_RETENTION_YEARS = 7;

// Mask sensitive data in logs
function maskFinancialData(data: any): any {
  // Mask amounts in logs (show only last 2 digits)
  // Mask GSTIN/PAN partially
}
```
...
```typescript
// Prevent SQL injection in expense search
// Validate all numeric inputs
// Sanitize description and notes fields
// Validate file uploads for attachments (type, size)
```
