# Module 7: Inventory Management - Design Document
...
└────────┬────────┘       └─────────────────┘       └─────────────────┘
         ...
         ▼
┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│    Vendors      │──────<│ Purchase_Orders │>──────│  Goods_Receipt  │
│                 │  1:N  │                 │  1:N  │                 │
└─────────────────┘       └─────────────────┘       └─────────────────┘

┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│ Stock_Movement  │       │ Stock_Transfer  │       │  Stock_Audit    │
│                 │       │                 │       │                 │
└─────────────────┘       └─────────────────┘       └─────────────────┘
```
...
interface CreateGoodsReceiptResponse {
  success: boolean;
  data: {
...
interface RecordConsumptionRequest {
  ...
  items: {
    productId: string;
    quantity: number;
    reasonCode: "service" | "sample" | "demo" | "wastage" | "other";
    reasonDescription?: string;
...
interface CreateStockTransferResponse {
  success: boolean;
  data: {
...
interface ReceiveStockTransferRequest {
  items: {
...
interface ReceiveStockTransferResponse {
  success: boolean;
  data: {
    transfer: StockTransfer;
    discrepancies: {
...
interface StartStockAuditRequest {
  branchId: string;
  auditType: AuditType;
  categoryId?: string; // For partial/category audits
  notes?: string;
}

interface StartStockAuditResponse {
  success: boolean;
  data: {
    audit: StockAudit;
    items: StockAuditItem[];
...
// PATCH /api/v1/stock-audits/:id/items
interface UpdateAuditCountsRequest {
  items: {
    auditItemId: string;
    physicalQuantity: number;
  }[];
...
// POST /api/v1/stock-audits/:id/post
interface PostAuditAdjustmentsRequest {
  items: {
    auditItemId: string;
    adjustmentReason: string;
  }[];
...
interface PostAuditAdjustmentsResponse {
  success: boolean;
  data: {
    audit: StockAudit;
...
class StockService {
  ...
  private async updateStockSummary(
    ...
    const quantityOnHand = batches.reduce(
...
class PurchaseOrderService {
  ...
  async createPurchaseOrder(
    ...
    // Generate PO number
    const poNumber = await this.generatePONumber(request.branchId);
    ...
    for (const item of itemsWithTax) {
      await this.db.purchaseOrderItems.create({
        tenantId: this.tenantId,
  ...
  /**
   * Get reorder suggestions based on low stock and consumption
   */
  async getReorderSuggestions(branchId: string): Promise<ReorderSuggestion[]> {
    // Get low stock items
    const lowStockItems = await this.db.stockSummary.findMany({
      where: { branchId, isLowStock: true },
      include: { product: true },
    });
    ...
    for (const item of lowStockItems) {
      // Get consumption trend (last 30 days)
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

      const consumption = await this.db.stockMovements.aggregate({
        where: {
          branchId,
          productId: item.productId,
          movementType: {
            in: [StockMovementType.CONSUMPTION, StockMovementType.SALE],
          },
          movementDate: { gte: thirtyDaysAgo.toISOString().split("T")[0] },
        },
        _sum: { quantity: true },
      });

      const monthlyConsumption = Math.abs(consumption._sum.quantity || 0);
      const dailyConsumption = monthlyConsumption / 30;

      // Get preferred vendor
      const vendorProduct = await this.db.vendorProducts.findFirst({
        where: { productId: item.productId, isPreferred: true },
        include: { vendor: true },
      });

      // Calculate suggested quantity (cover 45 days + buffer)
      const suggestedQuantity = Math.ceil(dailyConsumption * 45);
      ...
      suggestions.push({
        ...
        monthlyConsumption,
        suggestedQuantity,
        preferredVendor: vendorProduct?.vendor,
        lastPurchasePrice: vendorProduct?.lastPurchasePrice,
      });
    }
...
class StockTransferService {
  ...
  async approveAndDispatch(
    ...
    if (transfer.status !== TransferStatus.REQUESTED) {
      throw new BadRequestError(
        "INVALID_STATUS",
        "Transfer is not in requested status",
      );
    }
    ...
    return this.db.transaction(async (tx) => {
      ...
      for (const item of transfer.items!) {
        await this.stockService.consumeStock(
          ...
          "transfer_out",
          `Transfer to branch ${transfer.destinationBranchId}`,
          "transfer",
          transfer.id,
          userId,
        );
        ...
        await tx.stockTransferItems.update(item.id, {
      ...
      // Notify destination branch
      await this.notificationService.notifyTransferDispatched(updatedTransfer);
  ...
  async receiveTransfer(
    ...
    return this.db.transaction(async (tx) => {
      ...
      for (const receivedItem of request.items) {
        ...
        const discrepancy =
          (transferItem.dispatchedQuantity || 0) -
...
- Update physical counts for audit items
  \*/
  async updateAuditCounts(
  auditId: string,
  items: { auditItemId: string; physicalQuantity: number }[],
  userId: string
  ): Promise<StockAudit> {
  ...
  for (const item of items) {
  const auditItem = await this.db.stockAuditItems.findUnique({
  where: { id: item.auditItemId },
  });

      if (!auditItem || auditItem.auditId !== auditId) continue;

      const variance = item.physicalQuantity - auditItem.systemQuantity;
      const varianceValue = variance * auditItem.unitCost;

      await this.db.stockAuditItems.update(item.auditItemId, {
  ...
  return this.db.stockAudits.findUnique({ where: { id: auditId } })!;

}

/\*\*

- Complete audit (mark all items counted)
  \*/
  async completeAudit(auditId: string, userId: string): Promise<StockAudit> {
  const audit = await this.db.stockAudits.findUnique({
  where: { id: auditId },
  include: { items: true },
  });
  ...
  const uncountedItems = audit.items!.filter(
  ...
  const totalVarianceValue = audit.items!.reduce(
  (sum, item) => sum + Math.abs(item.varianceValue || 0),
  0
  );
  const shrinkageValue = audit.items!
  .filter((item) => (item.varianceQuantity || 0) < 0)
  .reduce((sum, item) => sum + Math.abs(item.varianceValue || 0), 0);

  return this.db.stockAudits.update(auditId, {
...
it('should generate sequential PO number');
it('should calculate GST amounts correctly');
it('should set pending quantity equal to ordered quantity');
});

describe('getReorderSuggestions', () => {
it('should return low stock items');
it('should calculate suggested quantity based on consumption');
it('should include preferred vendor information');
});
});
...
it('should require all items to be counted');
it('should calculate variance and shrinkage values');
});
