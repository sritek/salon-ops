# Module 4: Services & Pricing - Design Document
...
```
┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│ Service_Category│──────<│    Service      │>──────│ Service_Variant │
│                 │  1:N  │                 │  1:N  │                 │
└─────────────────┘       └────────┬────────┘       └─────────────────┘
                                   │
                    ┌──────────────┼──────────────┐
                    ...
                    ▼              ▼              ▼
          ┌─────────────┐  ┌─────────────┐  ┌─────────────┐
          │ Branch_Price│  │  Add_Ons    │  │ Consumables │
          │  Override   │  │             │  │   Mapping   │
          └─────────────┘  └─────────────┘  └─────────────┘

┌─────────────────┐       ┌─────────────────┐
│  Combo_Service  │──────<│ Combo_Items     │
│                 │  1:N  │                 │
└─────────────────┘       └─────────────────┘
```
  ...
  async getEffectiveCommission(
    ...
    if (branchPrice?.commissionType && branchPrice?.commissionValue !== null) {
      return {
        ...
        assistantValue: service.assistantCommissionValue, // No branch override for assistant
        source: "branch",
      };
    }
