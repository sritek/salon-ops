# Module 12: Online Booking - Design Document
...
```
┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│ Booking_Config  │       │ Booking_Page    │       │ Online_Booking  │
│                 │       │                 │       │                 │
└─────────────────┘       └─────────────────┘       └────────┬────────┘
                                                             │
                                                             │ 1:N
                                                             ▼
┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│ Booking_Service │       │ Booking_Payment │       │   Slot_Lock     │
│                 │       │                 │       │                 │
└─────────────────┘       └─────────────────┘       └─────────────────┘

┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│ Abandoned_      │       │ Booking_        │       │ Blacklisted_    │
│ Booking         │       │ Analytics       │       │ Customer        │
└─────────────────┘       └─────────────────┘       └─────────────────┘
```
...
```typescript
// GET /api/v1/public/booking/:slug/slots
interface GetAvailableSlotsRequest {
  branchId: string;
  date: string;
  serviceIds: string[];
  variantIds?: string[];
  stylistId?: string; // Optional: filter by specific stylist
}
...
```typescript
// POST /api/v1/public/booking/:slug/lock-slot
...
interface CreateOnlineBookingRequest {
  branchId: string;
...
interface CreateOnlineBookingResponse {
success: boolean;
data: {
...
```typescript
// POST /api/v1/public/booking/:slug/payment
...
```typescript
// POST /api/v1/public/booking/reschedule/:ref
...
```typescript
// POST /api/v1/public/booking/cancel/:ref
...
```typescript
// POST /api/v1/public/booking/:slug/calculate
...
class AvailabilityService {
  ...
  async getAvailableSlots(
    ...
    const slots = this.generateTimeSlots(workingHours.open, workingHours.close, 30); // 30-min intervals
...
- Check if a specific slot is available
  ...
  const hoursUntilSlot = (slotTime.getTime() - now.getTime()) / (1000 _ 60 _ 60);
...
- Cancel a booking
  ...
  const hoursUntilBooking = (bookingDateTime.getTime() - Date.now()) / (1000 _ 60 _ 60);
...
- Create appointment from online booking
  \*/
  private async createAppointment(booking: OnlineBooking): Promise<Appointment> {
  const services = await this.bookingServiceRepo.findByBookingId(booking.id);
...
class BookingPaymentService {
  ...
  async verifyPayment(
    ...
    // Create appointment if auto-confirm
...
- Process wallet payment
  ...
  booking.customerId!,
  walletAmount,
  `Online booking prepayment - ${booking.bookingReference}`
  );
...
- Process package credit redemption
  \*/
  async processPackageCredits(
  booking: OnlineBooking,
  packageRedemptions: { packageId: string; serviceId: string }[]
  ): Promise<void> {
  for (const redemption of packageRedemptions) {
  await this.packageService.redeemCredit(
...
class LeadCaptureService {
  ...
  async trackAbandonment(
    ...
    if (existing) {
      // Update with latest step (only if progressed further)
      const stepOrder = Object.values(AbandonmentStep);
      if (
        stepOrder.indexOf(step) >
        stepOrder.indexOf(existing.abandonmentStep as AbandonmentStep)
      ) {
   ...
   */
  async getForFollowup(tenantId: string): Promise<AbandonedBooking[]> {
    const config = await this.configService.getConfig(tenantId);
    const cutoffTime = new Date();
    cutoffTime.setHours(
      cutoffTime.getHours() - config.abandonmentFollowupHours,
    );

    return this.abandonedRepo.findForFollowup(tenantId, cutoffTime);
  }
  ...
  async sendFollowup(abandonedBooking: AbandonedBooking): Promise<void> {
    if (!abandonedBooking.customerPhone) {
      return; // Can't follow up without phone
    }

    // Check if customer has since booked
    ...
    // Send follow-up message via marketing module
...
class FraudProtectionService {
  ...
  async isBlacklisted(
    ...
    if (customerId) {
      const byCustomer = await this.blacklistRepo.findByCustomer(
        tenantId,
        customerId,
      );
    ...
    if (customerId) {
      const noShowCount = await this.appointmentService.getNoShowCount(
        customerId,
        90,
      ); // Last 90 days
      if (noShowCount >= 3) {
  ...
  async removeFromBlacklist(
    ...
    await this.blacklistRepo.update(blacklistId, {
      isActive: false,
  ...
  async checkRateLimit(
    ...
    if (attempts === 1) {
      await this.cache.expire(key, 3600); // 1 hour window
    }
  ...
  async detectSuspiciousActivity(
    ...
    // Check multiple bookings from same IP
    const ipBookings = await this.bookingRepo.countByIpToday(
      tenantId,
      ipAddress,
    );
    if (ipBookings > 5) {
    ...
    const recentAttempts = await this.bookingRepo.countRecentByPhone(phone, 10); // Last 10 minutes
    if (recentAttempts > 3) {
...
"online_booking.created"; // New booking created
"online_booking.confirmed"; // Booking confirmed (manual or auto)
"online_booking.cancelled"; // Booking cancelled
"online_booking.rescheduled"; // Booking rescheduled
"online_booking.completed"; // Booking marked complete (via appointment)
"online_booking.no_show"; // Customer no-show (via appointment)
"online_booking.payment_completed"; // Prepayment successful
"online_booking.payment_failed"; // Prepayment failed
"online_booking.refund_processed"; // Refund processed

// Slot Events
"slot.locked"; // Slot temporarily locked
"slot.released"; // Slot lock expired/released

// Lead Events
"lead.abandoned"; // Booking abandoned
"lead.followup_sent"; // Follow-up message sent
"lead.converted"; // Abandoned lead converted

// Fraud Events
"fraud.customer_blacklisted"; // Customer added to blacklist
"fraud.suspicious_activity"; // Suspicious activity detected
```
...
- Use materialized view for availability calculation
- Batch fetch services and variants
- Preload staff schedules for date range
...
async cleanupExpiredLocks(): Promise<void> {
  await this.slotLockRepo.deleteExpired();
}
```
...
// Every 15 minutes
@Cron('*/15 * * * *')
async processAbandonmentFollowups(): Promise<void> {
  const tenants = await this.tenantService.getActiveWithOnlineBooking();

  for (const tenant of tenants) {
    const abandoned = await this.leadService.getForFollowup(tenant.id);
    for (const booking of abandoned) {
      await this.leadService.sendFollowup(booking);
    }
  }
}
```
...
async aggregateDailyAnalytics(): Promise<void> {
  ...
  for (const tenant of tenants) {
    await this.analyticsService.aggregateDaily(tenant.id, yesterday);
  }
}
```
...
// Every hour
@Cron('0 * * * *')
async remindPendingConfirmations(): Promise<void> {
  const pendingBookings = await this.bookingRepo.findPendingOlderThan(30); // 30 minutes

  for (const booking of pendingBookings) {
    await this.notificationService.sendPendingConfirmationReminder(booking);
  }
}
```
...
This design document covers the complete Online Booking module (Module 12) including:

- Public booking page configuration and branding
- Service selection and display
- Real-time availability engine (reusing Module 2 logic)
- Stylist selection
- Customer identification and profile linking
- Pricing transparency with membership/package preview
- Prepayment and deposit handling
- Booking confirmation flow (auto and manual)
- Modification and cancellation with refunds
- Walk-in conflict prevention via slot locking
- Multi-branch booking support
- Promo code and discount integration
- WhatsApp notifications
- Lead capture and abandonment tracking
- Fraud and abuse protection
- Manager controls and emergency shutdown
