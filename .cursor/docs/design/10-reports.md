# Module 10: Reports & Analytics - Design Document
...
```
┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│ Report_Config   │       │  Data_Snapshot  │       │ Snapshot_Metric │
│                 │       │                 │──────<│                 │
└─────────────────┘       └─────────────────┘  1:N  └─────────────────┘

┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│ Alert_Config    │       │  Alert_History  │       │ Scheduled_Report│
│                 │──────<│                 │       │                 │
└─────────────────┘  1:N  └─────────────────┘       └─────────────────┘

┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│ Report_Export   │       │ Saved_Filter    │       │ Dashboard_Widget│
│                 │       │                 │       │                 │
└─────────────────┘       └─────────────────┘       └─────────────────┘
```
...
recipients JSONB NOT NULL, -- [{type: 'email', value: 'x@y.com'}, {type: 'whatsapp', value: '+91...'}]
...
soldCount: number;
soldValue: number;
liability: number; // sold - redeemed
expiryLoss: number; // credits expired unused
};
...
interface StaffPerformanceReportRequest {
  startDate: string;
  endDate: string;
  branchId?: string;
  staffId?: string;
...
interface StaffPerformanceReportResponse {
  success: boolean;
...
```typescript
// GET /api/v1/reports/compare/period
...
interface CreateScheduledReportRequest {
  ...
  deliveryMethod: DeliveryMethod;
  recipients: { type: "email" | "whatsapp"; value: string }[];
  exportFormat: ExportFormat;
}
...
```typescript
// PATCH /api/v1/alerts/config/:type
interface UpdateAlertConfigRequest {
  isEnabled?: boolean;
  thresholdValue?: number;
  thresholdType?: ThresholdType;
  comparisonPeriod?: ComparisonPeriod;
...
interface UpdateAlertConfigResponse {
  success: boolean;
  data: {
...
```typescript
// GET /api/v1/reports/drill-down/:metric
interface DrillDownRequest {
  metric: string; // e.g., 'revenue', 'appointments', 'discounts'
  startDate: string;
  endDate: string;
  branchId?: string;
  groupBy?: string; // e.g., 'service', 'staff', 'day'
}
...
```typescript
// GET /api/v1/reports/source/:metric
interface SourceTraceRequest {
  metric: string;
  startDate: string;
  endDate: string;
  branchId?: string;
...
interface SourceTraceResponse {
  ...
  data: {
    ...
    transactions: {
      id: string;
      type: string; // invoice, expense, etc.
      date: string;
      reference: string;
      amount: number;
...
class ReportService {
  ...
  async getRevenueReport(
    ...
    const { startDate, endDate, branchId, groupBy, includeComparison } =
      request;
    ...
    if (includeComparison) {
      ...
      data.comparison = {
        ...
        changeAmount:
          data.summary.totalRevenue - comparisonData.summary.totalRevenue,
    ...
    // Log report access
    await this.logReportAccess(userId, ReportType.REVENUE, {
      startDate,
      endDate,
      branchId,
    });
  ...
  private shouldUseSnapshots(startDate: string, endDate: string): boolean {
    ...
    // Use live data for today and yesterday
    // Use snapshots for older data
  ...
  /**
   * Get revenue from snapshots
   */
  private async getRevenueFromSnapshots(
    startDate: string,
    endDate: string,
    branchId?: string,
  ): Promise<RevenueReport> {
    const snapshots = await this.snapshotRepo.findByDateRange(
      SnapshotType.DAILY_REVENUE,
      startDate,
      endDate,
      branchId,
    );
    ...
    const summary = this.aggregateSnapshotMetrics(snapshots);

    return {
      period: { startDate, endDate },
      branchId,
      summary,
      dataFreshness: snapshots[0]?.createdAt || new Date(),
    };
  }
}
```
...
- Stylist dashboard - personal metrics
  ...
  const todayAppointments = await this.appointmentService.getByStaff(
  user.id, today, today
  );

  // Get earnings
  const monthEarnings = await this.billingService.getStaffEarnings(
...
- Receptionist dashboard - operational focus
  \*/
  private async getReceptionistDashboard(user: User): Promise<any> {
  const today = new Date().toISOString().split('T')[0];
  const branchId = user.branchId;

  // Get today's appointments
  const appointments = await this.appointmentService.getByBranch(branchId, today);

  // Get walk-in queue
  const walkInQueue = await this.appointmentService.getWalkInQueue(branchId);

  // Get pending bills
  const pendingBills = await this.billingService.getPendingBills(branchId);

  // Get today's sales
  const todaySales = await this.billingService.getRevenue(branchId, today, today);
...
class AlertService {
  ...
  private async getAlertMetrics(
    ...
    branchId: string,
    comparisonPeriod?: ComparisonPeriod
  ): Promise<{ currentValue: number; comparisonValue?: number }> {
    const today = new Date().toISOString().split('T')[0];

    switch (alertType) {
      case AlertType.REVENUE_DROP:
        const currentRevenue = await this.billingService.getRevenue(branchId, today, today);
        const comparisonDate = this.getComparisonDate(today, comparisonPeriod);
        const prevRevenue = await this.billingService.getRevenue(branchId, comparisonDate, comparisonDate);
        return { currentValue: currentRevenue.total, comparisonValue: prevRevenue.total };

      case AlertType.WASTAGE_HIGH:
        const wastage = await this.inventoryService.getWastagePercent(branchId, today);
        return { currentValue: wastage };

      case AlertType.DISCOUNT_HIGH:
        const discountPercent = await this.billingService.getDiscountPercent(branchId, today);
        return { currentValue: discountPercent };

      case AlertType.NOSHOW_HIGH:
        const noShowRate = await this.appointmentService.getNoShowRate(branchId, today);
        return { currentValue: noShowRate };
...
- Evaluate threshold condition
  ...
  // For revenue drop, we check if drop exceeds threshold
  if (alertType === AlertType.REVENUE_DROP) {
  return changePercent <= -threshold;
  }

  // For other metrics, we check if increase exceeds threshold
  return changePercent >= threshold;
...
- Create alert and send notifications
  ...
  ? ((currentValue - comparisonValue) / comparisonValue) _ 100
  : undefined;
...
class ScheduledReportService {
  ...
  async processDueReports(): Promise<void> {
    const now = new Date();
    const dueReports = await this.scheduledReportRepo.findDue(now);

    for (const report of dueReports) {
      try {
        await this.runScheduledReport(report);
      } catch (error) {
        await this.handleReportFailure(report, error);
      }
    }
  }
  ...
  private async runScheduledReport(report: ScheduledReport): Promise<void> {
    ...
    const dateRange = this.calculateDateRange(report.scheduleType);
    ...
    for (const recipient of report.recipients) {
      if (recipient.type === "email") {
        ...
        await this.whatsappService.sendReportMessage(
          recipient.value,
          report.name,
          exportResult.data.fileUrl,
        );
    ...
    const nextRunAt = this.calculateNextRunTime(report);
   ...
   */
  private calculateDateRange(scheduleType: ScheduleType): DateRange {
...
```typescript
interface BillingReportIntegration {
  getRevenue(branchId: string | null, startDate: string, endDate: string): Promise<RevenueData>;
  getRevenueByBranch(startDate: string, endDate: string): Promise<BranchRevenue[]>;
  getRevenueByService(branchId: string | null, startDate: string, endDate: string): Promise<ServiceRevenue[]>;
  getPaymentMethodBreakdown(branchId: string | null, startDate: string, endDate: string): Promise<PaymentBreakdown[]>;
  getDiscountPercent(branchId: string, date: string): Promise<number>;
  getStaffEarnings(staffId: string, startDate: string, endDate: string): Promise<StaffEarnings>;
  getPendingBills(branchId: string): Promise<Invoice[]>;
}
````

### 2. Appointment Module Integration

```typescript
interface AppointmentReportIntegration {
  getAppointmentStats(
    branchId: string | null,
    startDate: string,
    endDate: string,
  ): Promise<AppointmentStats>;
  getNoShowRate(branchId: string, date: string): Promise<number>;
  getPeakHours(
    branchId: string | null,
    startDate: string,
    endDate: string,
  ...
  getWalkInQueue(branchId: string): Promise<Appointment[]>;
}
```
...
```typescript
interface CustomerReportIntegration {
  getCustomerStats(
    branchId: string | null,
    startDate: string,
    endDate: string,
  ...
  getChurnRiskCustomers(branchId: string | null): Promise<ChurnRiskCustomer[]>;
  getTopCustomers(
    branchId: string | null,
    startDate: string,
    endDate: string,
  ...
  getLoyaltyLiability(): Promise<LoyaltyLiability>;
}
```
...
```typescript
interface InventoryReportIntegration {
  ...
  getWastagePercent(branchId: string, date: string): Promise<number>;
  getDeadStock(branchId: string, unusedDays: number): Promise<DeadStock[]>;
  getNearExpiryStock(
    branchId: string,
    daysToExpiry: number,
  ): Promise<NearExpiryStock[]>;
}
```

### 5. Expense Module Integration

```typescript
interface ExpenseReportIntegration {
  getExpenseTotal(
    branchId: string | null,
    startDate: string,
    endDate: string,
  ...
  getPendingPayables(): Promise<number>;
  getBudgetVsActual(
    branchId: string,
    year: number,
    month: number,
...
### 6. Membership/Package Module Integration

```typescript
interface MembershipPackageReportIntegration {
  getMembershipStats(
    branchId: string | null,
    startDate: string,
    endDate: string,
...
describe("Report Integration", () => {
  it("should generate revenue report matching billing data");
  it("should generate P&L report with correct calculations");
  it("should send scheduled report on time");
  it("should trigger and deliver alerts");
  it("should drill down from summary to detail");
});
```
...
-- Export audit
CREATE INDEX idx_exports_user ON report_exports(generated_by, generated_at);
```
...
GROUP BY tenant_id, branch_id, date;
...
// Stylist can only see their own performance
// Receptionist can only see operational reports for their branch
...
// Sensitive report access logging
const SENSITIVE_REPORTS = [ReportType.PL, ReportType.GST];
```
...
'0 5 1 * *'  -> archiveOldSnapshots()         // Archive snapshots older than retention period
